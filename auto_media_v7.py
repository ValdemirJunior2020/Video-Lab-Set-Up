...
{script}"""
    try:
        ...
        for i, row in enumerate(rows[:desired], 1):
            ...
            scenes.append({
                ...
                "estimated_duration": max(3, float(row.get("estimated_duration") or clip_seconds)),
            })
        if scenes:
            return scenes
    ...
    parts = [x.strip() for x in re.split(r"(?<=[.!?])\s+", script) if x.strip()]
    return [{
        "scene_number": i + 1,
        "narration": p,
...
def _pexels_search(query: str, limit: int = 8):
    ...
    try:
        r = _SESSION.get(
            PEXELS_VIDEO_SEARCH,
            params={"query": query, "per_page": min(15, limit), "orientation": "landscape"},
...
def download_for_script(task_id: str, script: str, target_minutes: int, clip_seconds: int):
    ...
    for scene in scenes:
        ...
        item, source = chosen
        dest = output_dir / f"scene-{scene['scene_number']:03d}{source.suffix.lower()}"
